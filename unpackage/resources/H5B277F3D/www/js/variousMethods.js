	...
	function homePageBottomList(data){
		...
		if (data.code == 200) {
			...
			for(j=0;j<dataBottomLis.length;j++){
				...
				htmlThree.push('-- <span class="hostClass">'+dataBottomLis[j].name+'</span> --');
				htmlThree.push('</div>');
				...
				for(i=0;i<dataBottomLis[j].goodsList.length;i++){
					...
					htmlThree.push('<div><span class="gotoDitails" data-inventory=' + row.inventory + '  data-id='+row.id+'>￥'+ row.price+'</span><i class="fa fa-plus"  spId='+ row.pecificationId+' data-idCart='+ row.id+'></i></div></div>');
				}
				//如果是奇数 在最后加个查看更多的图片
